...
class QuestionClassifier:
    def __init__(self):
        ...
        self.child_wds = ['儿童', '小孩', '小朋友', '小宝宝', '小孩子', '小孩儿']
        self.region_words = set(
            self.department_wds + self.disease_wds + self.check_wds + self.drug_wds + self.food_wds + self.producer_wds + self.symptom_wds + self.child_wds)
        ...
        self.prevent_qwds = ['预防', '防范', '抵制', '抵御', '防止', '躲避', '逃避', '避开', '免得', '逃开', '避开',
                             ...
                             '怎样才可不', '怎么才可不', '咋样才可不', '咋才可不', '如何可不', '预防措施', '预防方法',
                             '如何预防', '怎么预防', '怎样预防']
        ...
        # 新增
        self.producer_qwds = ['生产商', '厂家', '厂商', '生产企业']
        self.compare_qwds = ['多还是', '哪个多', '哪个的数量更多', '哪个更多', '哪个大', '哪个多一些', '哪个多一些',
                             '数量多', '数量更多']
        self.can_eat_qwds = ['能否吃', '能不能吃', '可以吃', '能吃', '可不可以吃']
        self.otherdrug_qwds = ['还会生产', '还生产', '其他药物']
        self.department_qwds = ['属于哪个科室', '属于什么科', '哪个科室']
    ...
    def classify(self, question):
        ...
        # 某病能否吃某食物
        if self.check_words(self.can_eat_qwds, question) and 'food' in types and 'disease' in types:
            question_type = 'disease_food_can_eat'
            question_types.clear()
            question_types.append(question_type)

        # 比较两食物忌口疾病数量
        if self.check_words(self.compare_qwds, question) and 'food' in types:
            question_type = 'food_not_disease_compare'
            question_types.clear()
            question_types.append(question_type)

        # 比较两药品生产商数量
        if self.check_words(self.compare_qwds, question) and 'drug' in types:
            question_type = 'drug_producer_compare'
            question_types.clear()
            question_types.append(question_type)

        # 易感疾病检查
        if self.check_words(self.easyget_qwds + self.check_qwds, question) and 'disease' in types:
            question_type = 'disease_easyget_check'
            question_types.clear()
            question_types.append(question_type)

        # 易感疾病预防
        if self.check_words(self.easyget_qwds + self.prevent_qwds, question):
            if 'disease' in types:
                question_type = 'disease_easyget_prevent'
            elif 'child' in types:
                question_type = 'child_easyget_prevent'
            question_types.clear()
            question_types.append(question_type)

        # 药商还会生产哪些药品
        if self.check_words(self.otherdrug_qwds, question) and 'drug' in types:
            question_type = 'producer_other_drug'
            question_types.clear()
            question_types.append(question_type)

        # 并发症属于哪个科室
        if self.check_words(self.acompany_qwds + self.department_qwds, question) and 'disease' in types:
            question_type = 'disease_acompany_department'
            question_types.clear()
            question_types.append(question_type)
    ...
    def build_wdtype_dict(self):
        ...
        for wd in self.region_words:
            ...
            if wd in self.child_wds:
                wd_dict[wd].append('child')
        return wd_dict
